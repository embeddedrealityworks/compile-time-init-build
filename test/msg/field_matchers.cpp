...
TEST_CASE("matcher description", "[field matchers]") {
    ...
    STATIC_CHECK(desc.str == "test_field < 0x5"_ctst);
}
...
TEST_CASE("matcher description of match", "[field matchers]") {
    ...
    STATIC_CHECK(desc.str == "test_field (0x{:x}) < 0x5"_ctst);
    STATIC_CHECK(desc.args == stdx::tuple{1});
}
...
TEST_CASE("matcher description (enum field)", "[field matchers]") {
    ...
    constexpr auto desc = m.describe();
    STATIC_CHECK(desc.str == "enum_field < C"_ctst);
}
...
TEST_CASE("matcher description of match (enum field)", "[field matchers]") {
    ...
    constexpr auto desc = m.describe_match(msg_data{0x01ff'ffff});
    STATIC_CHECK(desc.str == "enum_field ({}) < C"_ctst);
    STATIC_CHECK(desc.args == stdx::tuple{E::B});
}
...
TEST_CASE("less_than X implies less_than Y (X == Y)", "[field matchers]") {
    ...
    STATIC_CHECK(match::implies(m, n));
}
...
TEST_CASE("equal_to X and equal_to Y is false (X != Y)", "[field matchers]") {
    ...
    STATIC_CHECK(std::is_same_v<decltype(m), match::never_t const>);
}
...
TEST_CASE("predicate matcher description", "[field matchers]") {
    ...
    constexpr auto desc = m.describe();
    STATIC_CHECK(desc.str == "<predicate>(test_field)"_ctst);
}
...
TEST_CASE("predicate matcher description of match", "[field matchers]") {
    ...
    constexpr auto m =
        msg::pred_matcher_t<test_field, [](std::uint32_t) { return true; }>{};
    constexpr auto desc = m.describe_match(msg_data{0x01ff'ffff});
    STATIC_CHECK(desc.str == "<predicate>(test_field(0x{:x}))"_ctst);
    STATIC_CHECK(desc.args == stdx::tuple{1});
}
...
TEST_CASE("predicate matcher description of match (enum field)",
    ...
    constexpr auto desc = m.describe_match(msg_data{0x01ff'ffff});
    STATIC_CHECK(desc.str == "<predicate>(enum_field({}))"_ctst);
    STATIC_CHECK(desc.args == stdx::tuple{E::B});
}

namespace {
using matcher_field =
    field<"f", std::uint32_t>::located<at{0_dw, 31_msb, 0_lsb}>;
}

TEST_CASE("field matcher: equal_to", "[field matchers]") {
    using F = matcher_field::with_equal_to<17>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{17};
        STATIC_CHECK(m(msg));
    }
    {
        constexpr auto msg = std::array<uint32_t, 1>{18};
        STATIC_CHECK(not m(msg));
    }
}

TEST_CASE("field matcher: in", "[field matchers]") {
    using F = matcher_field::with_in<17, 18, 19>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{16};
        STATIC_CHECK(not m(msg));
    }
    {
        constexpr auto msg = std::array<uint32_t, 1>{17};
    ...
    {
        constexpr auto msg = std::array<uint32_t, 1>{19};
        STATIC_CHECK(m(msg));
    }
    {
        constexpr auto msg = std::array<uint32_t, 1>{20};
        STATIC_CHECK(not m(msg));
    }
}

TEST_CASE("field matcher: less_than", "[field matchers]") {
    using F = matcher_field::with_less_than<17>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{16};
        STATIC_CHECK(m(msg));
...
TEST_CASE("field matcher: greater_than", "[field matchers]") {
    using F = matcher_field::with_greater_than<17>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{17};
        STATIC_CHECK(not m(msg));
...
TEST_CASE("field matcher: less_than_or_equal_to", "[field matchers]") {
    using F = matcher_field::with_less_than_or_equal_to<17>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{16};
        STATIC_CHECK(m(msg));
...
TEST_CASE("field matcher: greater_than_or_equal_to", "[field matchers]") {
    using F = matcher_field::with_greater_than_or_equal_to<17>;
    constexpr auto m = F::matcher_t{};
    {
        constexpr auto msg = std::array<uint32_t, 1>{16};
        STATIC_CHECK(not m(msg));
